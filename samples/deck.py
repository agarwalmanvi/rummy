...
from detaprobc import *
from probaprobc import *
...
class Deck:
	def __init__(self):
		...
		valid_seq = copy.deepcopy(self.seq)
		self.seq = []			
		for i in valid_seq:
			for j in valid_seq:
				temp = [sum(x) for x in zip(i,j)]
				self.seq.append(temp)
		seqCopied = copy.deepcopy(self.seq)
		self.seq = []
		for i in seqCopied:
			if 2 not in i:
				self.seq.append(i)
		#here we changed valid_seq to contain all valid combinations of cards, and self.seq contains the valid sequences containing two combinations each
		...
		while True:
			if self.draw:
				...
				if draw_shuffle == 0:
					redist = copy.deepcopy(self.discard)
					while len(self.discard)>1:
						self.discard.remove(self.discard[-1])
					redist.remove(redist[0])
					self.draw = redist
					draw_shuffle = draw_shuffle+1
					continue
				else:
					self.winplay = 2
					break
